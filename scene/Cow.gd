...
func _physics_process(delta):
	var spawn_point = (get_tree().get_first_node_in_group("cow_spawn_point") as Marker2D).global_position - global_position
	if wander_time > 0:
		wander_time -= delta
		velocity = velocity.move_toward(wander_dir.normalized() * speed, max_speed)
		if wander_time <= 3:
			if spawn_point&&spawn_point.length() >= 75:
